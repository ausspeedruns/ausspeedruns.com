...
export const DonationIncentiveCreator = forwardRef<DonationIncentiveCreatorRef, DonationIncentiveCreatorProps>(
	(props, ref) => {
		...
		const [newIncentiveType, setNewIncentiveType] = useState<Omit<
			(typeof incentiveTypes)[number],
			"defaultValue"
		> | null>({ label: "Goal", value: "goal" });
		...
		function isIncentiveComplete(index?: number) {
			...
			if (index) {
				if (IncentiveIndexExists(index)) {
					...
					incentiveType =
						incentiveTypes.find((incentive) => incentive.value === allNewIncentives[index].type)?.value ??
						"";
		...
		useEffect(() => {
			if (IncentiveIndexExists(donationIncentiveStep)) {
				...
				setNewIncentiveType(
					incentiveTypes.find(
						(incentive) => incentive.value === allNewIncentives[donationIncentiveStep].type,
				...
				<Stack gap="medium">
					{props.donationIncentiveStrings[donationIncentiveStep] && (
						<>
							<h1 style={{ margin: 0, textAlign: "center" }}>
								{props.donationIncentiveStrings[donationIncentiveStep].game}
							</h1>
					...
					{newIncentiveType?.value === "goal" && (
						<>
							<FieldContainer>
								<FieldLabel>
									Goal ${(newIncentiveData as Goal["data"])?.goal?.toLocaleString() ?? 0}
								</FieldLabel>
								<TextInput
									disabled={!enabled}
									onChange={(e) =>
										setNewIncentiveData({ ...newIncentiveData, goal: parseInt(e.target.value) })
									}
									type="number"
