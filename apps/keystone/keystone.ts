		...
		db: {
			...
			extendPrismaSchema: (schema: any) => {
				return schema.replace(
					/(generator [^}]+)}/g,
					[
						'$1binaryTargets = ["native", "debian-openssl-1.1.x", "linux-musl-openssl-3.0.x", "linux-musl"]',
						"}",
					].join("\n"),
				);
			},
				...
				return {
					query: {
						accountVerification: graphql.field({
							...
							args: {
								where: graphql.arg({
									type: graphql.nonNull(base.inputObject("VerificationWhereUniqueInput")),
								}),
							},
							async resolve(source, args, context: Context) {
								...
								try {
									...
									const itemArr = await context
										.sudo()
										.db.Verification.findMany({ where: { code: { equals: args.where.code } } });
					...
					mutation: {
						...
						generateTicket: graphql.field({
							...
							async resolve(
								source,
								{ apiKey, event, method, numberOfTickets, stripeID, userID },
								context: Context,
							) {
								if (apiKey !== process.env.API_KEY) throw new Error("Incorrect API Key");

								// Check user is verified
								const userVerified = await context
									.sudo()
									.query.User.findOne({ where: { id: userID }, query: "verified" });
