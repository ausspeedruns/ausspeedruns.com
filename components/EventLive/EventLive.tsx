		...
		<div className={styles.eventLive}>
			...
			<div className={styles.onDeck}>
				...
				<div className={styles.columnRight}>
					<h4>Runners</h4>
					<h3>{eventQuery.data?.event.runs?.[currentRunIndex]?.runners.map((runner) => runner.username).join(', ') ?? 'Loading'}</h3>
				</div>
			</div>
			...
			<div className={styles.upcoming}>
				...
				<div className={styles.liveContent}>
					...
					<div className={styles.info}>
						...
						<span className={styles.subtitle}>{eventQuery.data?.event.runs?.[nextRunIndex]?.runners.length > nextRunIndex ? 'Runners' : 'Runner'}</span>
						<span>
							{eventQuery.data?.event.runs?.[nextRunIndex]?.runners.map((runner) => runner.username).join(', ') ?? 'Loading'}
						</span>
					</div>
					<div className={styles.link}>
